...
- 首屏允许用户手动设置并保存对战 ID（`public_id`），支持一键复制。
- 邀请时使用对战 ID 查找对方，而不是直接使用底层 UUID。

2. 对战邀请

- 输入对方对战 ID（`public_id`），先解析为用户 UUID，再创建 `pending` 邀请。
- 被邀请方实时收到邀请。
- 被邀请方可点击“同意”或“拒绝”。
...
- 15x15 棋盘，黑棋先手，建议使用 Canvas 渲染。
- 仅轮到自己时可落子。
- 每次落子写入 Supabase（`moves` 表），双方实时同步。
- 自动判定胜负（五子连线：横/竖/斜）。
- 对局结束后显示结果并禁止继续落子。
- 每回合增加倒计时，超时后自动随机选择一个合法空位落子。

4. 结束与重开规则

- 对局结束后显示 `Victory/Defeat/Draw`。
- 自动释放本局实时连接。
- 邀请输入框保留/回填上局对手 ID。
- 想继续对战必须重新发送邀请，禁止自动开始下一局。
...
- `profiles`：`id (uuid, pk)`、`public_id (text, unique)`、`created_at`
- `invites`：
  - `id (uuid, pk)`
  - `from_user (uuid)`
  - `to_user (uuid)`
...
- 对所有表开启 RLS。
- `profiles` 允许按 `public_id` 查询必要信息（用于发起邀请），但需限制敏感数据暴露。
- 邀请读写仅允许邀请双方访问。
- 对局读写仅允许黑白双方访问。
- 落子 insert policy：
  - 仅游戏参与者可写入。
...
- `auth`：匿名登录并缓存会话。
- `identity`：设置/保存/复制 `public_id`。
- `invite`：按 `public_id` 发送、订阅、同意/拒绝邀请。
...
- 顶部：当前用户 ID。
- 左侧：身份区域（设置自己的 `public_id`）。
- 右侧：发起邀请区域（输入对方 `public_id` + 发送）。
- 右侧：收到的邀请列表（同意/拒绝）。
...
- 两个浏览器（或隐私窗口）设置不同 `public_id` 后可互相邀请。
- 对方同意后立即进入同一局并实时落子。
- 非自己回合无法落子。
- 一方五连后正确结束对局。
- 每回合超时会自动随机合法落子。
